...
import { ThemedAlertDialog, type ThemedAlertButton } from './ThemedAlertDialog';
...
const shareActionsController = createShareActionsController({
  ...
  editShare: async (ledgerId: string, nextTitle: string) => {
    await appService.editShare({ ledgerId, title: nextTitle });
  },
});
...
export default function ShareDrawerContent({ onClose }: ShareDrawerContentProps): JSX.Element {
  ...
  async function saveParticipantEdit(participantId: string): Promise<void> {
    ...
    if (!trimmed) {
      setAlertDialog({ title: 'Rename participant', message: 'Participant name is required.', buttons: [{ label: 'OK' }] });
      return;
    }
    ...
    try {
      ...
      setAlertDialog({
        ...
        message: caught instanceof Error ? caught.message : 'Unable to rename participant.',
        buttons: [{ label: 'OK' }],
      });
    }
  }
  ...
  function confirmRemoveParticipant(participantId: string, displayName: string): void {
    setAlertDialog({
      ...
      message: `Remove ${displayName} from this share?`,
      buttons: [
        { label: 'Cancel' },
        {
          label: 'Remove',
          ...
          onPress: () => {
            void (async () => {
              try {
                ...
                setAlertDialog({
                  title: 'Remove participant',
                  message: caught instanceof Error ? caught.message : 'Unable to remove participant.',
                  buttons: [{ label: 'OK' }],
                });
              }
            })();
  ...
  async function runShareAction(action: string): Promise<void> {
    ...
    if (action === 'edit') {
      const share = shares.find((entry) => entry.ledgerId === selectedShareId);
      setEditingShareId(selectedShareId);
    ...
    setAlertDialog({
      title: 'Delete share',
      message: 'Delete this share permanently?',
      buttons: [
        { label: 'Cancel' },
        {
          label: 'Delete',
  ...
  async function saveShareEdit(ledgerId: string): Promise<void> {
    try {
      await shareActionsController.runAction('edit', ledgerId, editingShareTitle);
      ...
      setAlertDialog({
        title: 'Edit share',
        message: caught instanceof Error ? caught.message : 'Unable to edit share',
        buttons: [{ label: 'OK' }],
      });
    }
  }
  ...
  function onResetPress(): void {
    setAlertDialog({
      ...
      message: 'This erases all shares and expenses on this device.',
      buttons: [
        { label: 'Cancel' },
        {
          label: 'Delete all',
      ...
      >
        ...
        {activeShare ? (
          <Pressable
            ...
            accessibilityRole="button"
          >
            {editingShareId === activeShare.ledgerId ? (
              <View style={dynamicStyles.participantEditBlock}>
                <TextInput
                  value={editingShareTitle}
                  onChangeText={setEditingShareTitle}
                ...
                <View style={dynamicStyles.participantActionRow}>
                  <Pressable
                    onPress={() => {
                      void saveShareEdit(activeShare.ledgerId);
                    }}
                    style={[dynamicStyles.participantActionButton, dynamicStyles.participantActionPrimary]}
                    accessibilityRole="button"
                  >
                    <Text style={dynamicStyles.participantActionPrimaryText}>Save share</Text>
                  </Pressable>
                  <Pressable
                    onPress={() => {
                      setEditingShareId(null);
              ...
              {editingShareId === share.ledgerId ? (
                <View style={dynamicStyles.participantEditBlock}>
                  ...
                  <View style={dynamicStyles.participantActionRow}>
                    <Pressable
                      onPress={() => {
                        void saveShareEdit(share.ledgerId);
                      }}
                      style={[dynamicStyles.participantActionButton, dynamicStyles.participantActionPrimary]}
                      accessibilityRole="button"
                    >
        ...
        <View style={dynamicStyles.sectionHeaderRow}>
          ...
          <Pressable
            ...
            onPress={() => {
              if (!activeShare) {
                setAlertDialog({ title: 'Add participant', message: 'Create or select a share first.', buttons: [{ label: 'OK' }] });
                return;
              }
              router.push({
